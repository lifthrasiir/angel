...
const LogoAnimation: React.FC<LogoAnimationProps> = ({ width = '50px', height = '50px', color = 'black' }) => {
  const animationStyle = {
    animation: 'scaleXAnimation 60s linear infinite',
    animationDelay: '15s',
  };
  ...
  const keyframesStyle = `
    @keyframes scaleXAnimation {
      ...
      8.33% { transform: scaleX(-1); } /* 5s / 60s = 8.33% */
      50% { transform: scaleX(-1); }  /* 30s / 60s = 50% (5s anim + 25s pause) */
      58.33% { transform: scaleX(1); } /* 35s / 60s = 58.33% (30s + 5s anim) */
      100% { transform: scaleX(1); } /* 60s / 60s = 100% (35s + 25s pause) */
    }
  `;
  ...
  return (
    ...
    >
      ...
      {/* https://commons.wikimedia.org/wiki/File:Liberty-wing_-_Delapouite_-_game-icons.svg (CC-by 4.0) */}
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" style={{ width: '100%', height: '100%', ...animationStyle }}>
